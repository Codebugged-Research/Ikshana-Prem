...
def btn_clicked():
    ...
    if entry1.get() == 'Harish' and entry0.get() == 'harish@123':
    	...
    	# err_msg.config(textvariable = StringVar(canvas, value='error'))
...
logo = ImageTk.PhotoImage((Image.open("Button/login_page/logo.jpeg")).resize((500,500), Image.ANTIALIAS))
logo_bg = canvas.create_image(200, 250,image = logo)
line = canvas.create_line(450, 0, 450, 600)
...
canvas.create_text(
    738.5, 155.0,
    text = "Transforming Enterprise Vision at Scale",
    fill = "#000000",
    font = ("Roboto-Light", int(12.0)))

canvas.create_text(
    739.0, 80.5,
    text = "  Ikshana Dashboard",
    fill = "#4e6fe7",
